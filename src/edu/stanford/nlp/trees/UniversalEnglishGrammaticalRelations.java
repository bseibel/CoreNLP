...
public class UniversalEnglishGrammaticalRelations {
   ...
   */
    ...
    private static final List<GrammaticalRelation> values =
            Lists.newCopyOnWriteArrayList(Arrays.asList(
                    GOVERNOR,
                    DEPENDENT,
                    PREDICATE,
                    AUX_MODIFIER,
    ...
    private static final List<GrammaticalRelation> synchronizedValues = values;
    private static final List<GrammaticalRelation> unmodifiableSynchronizedValues = values;
    ...
    public static final Set<GrammaticalRelation> clauseRelations =
            Sets.newConcurrentHashSet(Arrays.asList(
                    CONJUNCT,
                    XCLAUSAL_COMPLEMENT,
                    CLAUSAL_COMPLEMENT,
                    CLAUSAL_MODIFIER,
    ...
    public static final Map<String, GrammaticalRelation> shortNameToGRel = new MapMaker().concurrencyLevel(32).makeMap();

    static {
        for (GrammaticalRelation gr : values()) {
            shortNameToGRel.putIfAbsent(gr.toString().toLowerCase(), gr);
        }
    }
...
//  }
    ...
    public static void threadSafeAddRelation(GrammaticalRelation relation) {
        values.add(relation);
        shortNameToGRel.putIfAbsent(relation.toString(), relation);
    }

    // the exhaustive list of conjunction relations
    private static final Map<String, GrammaticalRelation> conjs = new MapMaker().concurrencyLevel(32).makeMap();
    ...
    public static GrammaticalRelation getConj(String conjunctionString) {
        conjs.computeIfAbsent(
                conjunctionString,
                k -> {
                    GrammaticalRelation r = new GrammaticalRelation(
                    ...
                    threadSafeAddRelation(r);
                    return r;
                }
        );
        return conjs.get(conjunctionString);
    }

    // the exhaustive list of preposition relations
    private static final Map<String, GrammaticalRelation> nmods = new MapMaker().concurrencyLevel(32).makeMap();
    private static final Map<String, GrammaticalRelation> acls = new MapMaker().concurrencyLevel(32).makeMap();
    private static final Map<String, GrammaticalRelation> advcls = new MapMaker().concurrencyLevel(32).makeMap();
    ...
    /* Check for nmod subtypes which are not stored in the `nmods` map. */
        ...
        return nmods.get(prepositionString);
    }
